...
class Strato_Cron_Update_Deactivated_Plugins {

	public function __construct() {
		add_action( 'wp', array( $this, 'setup_schedule' ) );
		add_action( 'strato_assistant_cron_update_deactivated_plugins', array( $this, 'update_deactivated_plugins' ) );
	}

	public function setup_schedule() {
		$recurrence = 'daily';

		if ( ! wp_next_scheduled( 'strato_assistant_cron_update_deactivated_plugins' ) ) {
			wp_schedule_event( time(), $recurrence, 'strato_assistant_cron_update_deactivated_plugins' );
		}
	}
...
new Strato_Cron_Update_Deactivated_Plugins();
