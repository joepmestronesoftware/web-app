...
class Strato_Assistant_Config {
	...
	/**
	 * Strato_Assistant_Config constructor
	 */
	 ...
	 *
	 * @return Strato_Assistant_Config
	 */
	 ...
	 * Call: Strato_Assistant_Config::get()
	 *
	 * @param  string $param
	 ...
	 * Call: Strato_Assistant_Config::section()
	 *
	 * @param  string $section
	 * @return array
	 */
	...
	public static function set_paths( $config_paths = array() ) {
		...
		if ( ! empty( $config_paths ) ) {
			...
			self::$paths = array(
				1 => Strato_Assistant::get_plugin_dir_path() . 'config/main.ini',
				2 => Strato_Assistant::get_plugin_dir_path() . 'config/local.ini',
	...
	public function get_param( $param, $section = null, $fallback_param = null, $fallback_wording = null ) {
		...
		if ( get_option( 'strato_assistant_' . $param ) !== false ) {
			return get_option( 'strato_assistant_' . $param );
		}
	...
	private function replace_placeholders( $chain ) {
		...
		$market = Strato_Assistant::get_market();
		$chain = str_replace(
	...
	private function retrieve_db_params() {
		...
		foreach ( $this->config as $section => $params ) {
			foreach ( $params as $param => $value ) {
				...
				$wp_option_key = 'strato_assistant_config.' . $section . '.' . $param;
	...
	private function retrieve_cookie_params() {
		...
		if ( array_key_exists( 'features', $this->config ) && is_array( $this->config['features'] ) ) {
			foreach ( $this->config['features'] as $param => $value ) {
				...
				if ( isset( $_COOKIE[ 'strato-assistant-config-' . $param ] ) ) {
					$this->config['features'][ $param ] = filter_var( $_COOKIE[ 'strato-assistant-config-' . $param ], FILTER_SANITIZE_STRING );
				}
			}
		}
	}
